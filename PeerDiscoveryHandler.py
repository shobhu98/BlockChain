...
class PeerDiscoveryHandler():
    ...
    def handshakeMessage(self):
        """
        handshake message is sent to the connecting node that consist of sendConnector address and its peers with which it is connected
        :return:
        """
    ...
    def handleMessage(self, message):
        """
        if a new Node establishes a connection, add it in the peers list . Also if there is a node that is present in others peer list but not in current node list then establish a connection  with it
        as well.
        :param message:
        :return:
        """
